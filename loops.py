import sys
import re
from conditions import conditionVarify
from start import runner
from var import definer, rloops
...
def loopCh(scode):
        ...
        if ctx.startswith('jabtak ') or ctx.startswith('jabtak('):
    ...
    {ctx}\n{'    '+' '*ctx.find(rarg2[1:-1].strip(), len(qtxt)+4)+'^'*(len(rarg2[1:-1].strip()))}'''
            ...
            return True, qtxt, ctx
def whileInnit(scode):
    pos = 0
    ...
    {scode.strip().splitlines()[0]}\n{'    '+' '*(scode.strip().splitlines()[0].find('{:')+1)+'^'}
                    '''
                glti(er, 'GalatHaibhai: bhai \'{:\' ko \'}\' se close bhi krna hota h (line)')
        if (ans:=loopCh(code))[0]:
            cap = scode.splitlines()[1:linm+1]
        try:
            return tLines, ans[1], ans[2], '\n'.join(lines for lines in cap)
